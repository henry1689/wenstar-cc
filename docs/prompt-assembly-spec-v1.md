...
| inj-08 | L2 | 对话历史 | messages + kb 摘要 | 恒定 | ✅ **实际已收口**（V27批3：20 条上限 + 400 字符截断；V27批19 实测复核：`hist=67` 字符、`kb_echo=0`，3 条路径中 `process-stages.ts` 为死代码，活的仅 `_entityContextText` 且含 `!includes('关于你的知识库档案')` 去重守卫 —— 原「3 路径去重」待办据此关闭）|
...
### 批19 复核记录（2026-09-20）—— inj-08 关闭

**背景**：PAS 遗留清单长期挂着「inj-08 三路径去重待批4」。本次按「先量化再动手」复核，
**结论是该项实际已达成**，应关闭而非继续挂账：

| 复核项 | 实测 | 结论 |
|---|---|---|
| 历史注入量 | `hist=67` 字符 | 批3 的 20 条上限 + 400 字符截断已生效 |
| 重复回显 | `kb_echo=0` | 无重复注入 |
| 路径 1 `process-stages.ts:239` | 死代码（无生产者） | 不构成路径 |
| 路径 2 `chat.ts:1172` continuityParts | 活 | 唯一文本块路径 |
| 路径 3 `EntityContextBuilder:284` | 活（会晤专用，与 chat.ts 分支互斥） | 不重复 |
| KB 注入去重 | `1238` 行带 `!_entityContextText.includes('关于你的知识库档案')` 守卫 | **有守卫，不重复** |

**顺带发现**：`kb=6799` 占 L2 的 99%，**但这不是重复**，而是会晤模式确实需要的内容
（户口本 + 该实体 KB 档案）。若后续需压缩，应作为**独立课题**（内容必要性）而非"去重"。

**方法论记录**：本次复核没有直接动手改代码，而是先拿埋点数据（`[PromptBudget]`）核对，
确认"待办"描述的症状是否仍存在 —— 避免了又一次"在没测清前就动手"（参见批17/18 教训）。

### 批11 收口记录（2026-09-19）

**目标**：治源头 —— 减少新垃圾入 FG。

**根因**：`L3EntityAnnotator` 的 3 字滑窗只检查**前两字**是否为常见词
（`COMMON_WORDS_PERSON.has(token.substring(0,2))`），**无后两字检查** →
以常见词结尾的碎片漏过（“家死心/米人死/公室白”）。

**改动**：对称补全后两字检查（同一词表、同口径）。

**⚠️ 实测收益评估（诚实）**：
| 项 | 结果 |
|---|---|
| 真人零误伤 | ✅ 通过（8 个真名：熊梓铭/王全芬/林土锋/徐诗雨/宁清华/陈雪花/张小龙/罗权斌）|
| 表内词有效性 | ✅ 通过（“他应该/是时候/我但是”被拦）|
| **对现存垃圾的实际收益** | ❌ **≈ 0** —— 现存垃圾的后两字（想法/死心/人死/室白）**都不在词表** |

即：本改动是**对称补全 + 预防性**，**不能治存量**。真正的根治需要**词频/分词资源**
（或 LLM NER），已在下方列为待决策项。

**同时清障**：`grader-non-name-tail.test.ts` 的“历史垃圾候选拦截率”在 uuid 全量回填后
候选集变 0 → `0/0=NaN` 崩溃（非判据问题）。已加空集保护。
...
**🔍 待用户决策：FG 存量的“3 字噪声”根治方案**

实测 active 167 个 person 实体中 **3 字占 112（67%）**，且真名与噪声混杂：
```
真名: 熊梓铭 王全芬 林土锋 徐诗雨 宁清华 陈雪花 张小龙 徐东伟 警幻仙姑
噪声: 男朋友 后找男 明伶俐 谢想法 国读书 家死心 米人死
```
「明伶俐」vs「张小龙」**无法用字表区分** → 需选一条路：

| 方案 | 做法 | 成本 | 风险 |
|---|---|---|---|
| A. 词频/词典 | 内置常用词表反查「片段是否为词」| 中（需资源）| 低（可控误伤）|
| B. LLM NER | 调 LLM 判定候选是否为人名 | 高（每轮成本）| 低 |
| C. 二次确认 | L3 首次出现只记候选，二次出现才入 FG | 低 | 中（首次介绍的真人会延迟建档）|
| D. 维持现状 | 依赖下游（void 机制 + 摘要过滤，批10 已做）| 0 | 低（噪声仍在 FG）|

**我的建议**：先试 **A**（词频反查），因为它不影响实时链路、可离线批量重判存量；
若效果不足再考虑 B。
...
| 项 | 内容 |
|---|---|
| **定位** | m4 逐层埋点 → `batchProfile` 占 4551~6660ms；再单档案计时 → **`allProfileNames` 达 339 个**（含实体识别噪音碎片：「马上/干净/后我/段时间/家子/公室白」），339 × ~13ms ≈ 4.4s |
| **优化** | ① `FamilyGraph.getPersonProfileWithBio()` 一次节点查询产出 profile+bio（原两方法各查一次）；② **限量加载**档案；③ 分段埋点 + 单档案慢查询诊断 |
| **评审 P1-1（已修）** | 初版 `slice(0,60)` 会**静默截断家人块** —— 家人块内混有被误挂家族边的噪音（实测「马上」「秋节快」有家族边），噪音占名额会把真家人挤到 60 之后，而 `chat.ts` 只渲染 family_context 的档案字段 → LLM 对被挤掉者的外貌/年龄答「不知道」（不残缺回归）。**改为「家人全量 + 只截熟人 + 超限告警」** |
| **评审 P2-2（已修）** | 我原注释把 `rest=6031ms` 当作「2N 查询」的实锤是**错的**（rest 当时是未命名余量）；实测每档案均耗时不变，**真实增益全部来自「少加载」**（339→60），合并读取的贡献未获数据证实 |
| **评审 P2-4（已修）** | 埋点补齐：`decompose+uuid` 前置段；`fgSummary`/`filterFG` 改为**无条件** mark（原在分支内，另一分支会被并入 rest）|

**实测（干净环境 pm2 单实例）**：

| 指标 | 优化前 | 优化后 |
|---|---|---|
| batchProfile | 4551~6660ms | **1014~1318ms** |
| m4 total | 5674~14168ms | **1369~4937ms** |
| 端到端 | 24.4/15.5/15.4s | **15.1/7.8/6.9s** |

**暴露的存量数据质量问题（待单独立项）**：
- FG `nodes` 中 person 实体 **339 个**（家人实际 31、熟人 308），含大量识别噪音碎片
- 评审逐样本判定：一半是**存量未清**（`马上`/`干净`/`段时间`/`秋节快`/`舒服呀` 现行准入已能拦），
  一半是**准入判据失效**（`后我`/`家子`/`公室白`/`文屏障`/`家风月` —— 首字在姓氏表 + 长度≥3 的滑窗片段无任何判据）
- 结构性放大器：`GarbageEntityGuard` 的 `existingNames` 快路径使**历史垃圾名一旦入库即永久白名单**
- `getFamilySummary`/`getSocialSummary` 扫描 `SELECT * FROM nodes` **不过滤 `status='void'`** → 已回收实体仍占名额

**批8 建议**：① `filterFG` 的 N 次查询（实测 1964~3305ms，改后一度成为 m4 最大段）；
② `batchProfile` 改一次性 `WHERE name IN (...)`；③ 摘要侧加 `status<>'void'`；④ FG 噪音存量清理 + 准入判据补强
