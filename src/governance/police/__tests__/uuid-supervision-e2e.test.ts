...
describe('[监管 C] 任意称呼归一 —— 全名/小名/昵称都必须归一到同一 UUID', () => {
  ...
  /**
   * 2026-09-11 已修复（别名覆盖治理，通则化推导）：
   *   原先 王全芬 aliases=[] → 阿芬/全芬 无法归一（且正是另一窗口 P2-2 报告声称「已注册」
   *   却从未落地的那一项）。现由「数据驱动别名推导」一般规则自动登记：
   *     3 字姓名 → 派生 名1名2 / 阿+名2 / 小+名2，且需①在真实对话中出现过②不与其他 active 人重名
   *     ③不等于本名④不是类别级常用词（防「美容/清华」这类通用提及被错归一）
   *   别名覆盖：4/26(15.4%) → 12/26(46.2%)。
   */
  it('王全芬：全名 / 「阿芬」/「全芬」→ 同一 UUID', () => {
    if (!fg) return;
    const uuid = fg.getUUIDByName('王全芬');
    expect(uuid).toBeTruthy();
    expect(fg.getUUIDByName('阿芬'), '通则要求任意称呼归一（UUID 红线①）').toBe(uuid);
    expect(fg.getUUIDByName('全芬'), '通则要求任意称呼归一（UUID 红线①）').toBe(uuid);
  });
