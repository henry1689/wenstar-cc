...
### 🔴 铁律 0.4.1：UUID 管理通则 — 适用于所有人群（红线·不可逾越）

**依据**：2026-09-02 通则化治理决议（已清除徐诗雨/熊梓铭/徐诗韵三处个案硬编码）

**UUID 是系统管理每一个人的唯一手段（立法）**，人名只是兜底管理与交互便利。以下为强制通则，**适用于系统已有人群和未来新增人群**，不只针对某个人或某几人：

1. **任意称呼 → 归一化 → UUID**：用户用全名/小名/昵称/激情称呼（如"诗韵/韵韵/全芬/阿芬"）称呼任何人，必须先经 FG 别名映射（`getAllPersonNamesWithAliases`）归一化到**主名**，再 `getUUIDByName` 转 UUID。**禁止**只按主名精确匹配而漏掉简称/昵称。
2. **禁止个案硬编码**：禁止在代码中硬编码特定人名/UUID 的补丁逻辑（如"某实体去重""某实体补建档案"）。同类问题必须用通用规则覆盖**所有实体**（通则），例如：KB 档案去重按 belong_entity_uuid 分组通用处理，不用 `WHERE uuid='TXS-00000000X'`。
3. **新增人群自动适用**：任何新人物进入系统（登记 FG / 首次对话），必须自动获得完整的 UUID 链路（会晤进入→写入标注→检索驱动→闸门管控→档案动态采集），**不得要求额外写代码**。
4. **档案动态采集**：人物档案只能由 PAE/ProfileExtractor 从对话动态提取写入 FG properties，**禁止**在启动修复里写死档案内容。
5. **别名注册通则**：任何人的别名/昵称应注册到 FG `aliases` 字段（数据驱动），代码不得硬编码别名映射。
6. **触线即拦截**：遇到以下任一情况，先停手提醒用户，不得直接实施：
   - 新增代码需要硬编码特定人名/UUID 才能工作
   - 某功能只对个别实体生效而无法推广到所有实体
   - 某人的数据需要单独写补丁修复
   - 名称识别失败导致某人无法进入会晤/检索（应先查 FG 别名/滑窗通则，而非加个案分支）

**违抗后果**：违背通则的补丁=系统性债务，未来每个新人物都要再打一次补丁。宁可多做一步通则化，不写个案分支。
