...
> 注：V4.0 起，"FG" 缩写正式代表"太虚境户籍管理体系"（原"家族图谱"名称已废止）。FamilyGraph 为户籍管理体系的数据持久层实现。
> 代码位置：src/m4/household/FamilyGraph.ts（户籍管理体系目录）
> 文曲星·太虚境 硅基生命体 — 户籍管理系统
> 最后更新：2026-07-18
> 版本：V3.5
> 本次更新：fgIntegrityGuard 10项 + Dossier V6 七子卷 + UUID 纯流水号
> 法律依据：[《太虚境户籍管理法 V2.1》](../taixu-household-registration-law.md)
...
### 11.1 登记表（nodes 行级列）
uuid (TXS-9位流水号) / name (法定本名) / aliases (别名数组) / entity_source (五类) / category (A/B/C/D/E/F/G/H/X/S) / security_level (1/2/3) / status (active/dormant/archived/deceased) / family_gene / social_group_genes / created_at / updated_at

### 11.2 人生卷宗（Dossier 七子卷）
① basicInfo — 性别/出生年/出生地/学历
② selfProfile — 性格/外貌/习惯/喜恶/禁忌/语言/健康（替代旧 imageTraits + personalityPrefs + health + 散落 flat 字段）
③ socialIdentity — 职业时间线/婚恋时间线/当前职业快照（替代旧 occupation + relationMap 时序）
④ lifeMilestones — 人生重大事件编年表
⑤ boundDocuments — 典籍双向绑定清单
⑥ _changeHistory — V6 格式 {time, operation, field, before, after, reason, source}
⑦ misc — 兜底自由格式 JSON

旧 10 模块（contact / lifeResume / imageTraits / personalityPrefs / relationMap / familyNetwork / health / socialCapital / memoryAnchors）归档于 `_deprecated`，遵循卷宗只增不删。
...
每次系统启动时，FG 自动执行 10 项完整性检查。**任何一项不通过，系统降级运行并输出错误日志。** 检查失败意味着 FG 数据已被破坏，必须立即诊断。
...
| ⑤ entity_relations 无"姐妹"污染 | 信息输出，不阻断启动 | 手动清理 |
| ⑥ 所有人有姓名 | 节点数据不完整 | `ensurePersonProfile()` |
| ⑦ 全部节点有合法 UUID | 户籍编号缺失——门阀/检索异常 | `migrateToV3()` |
| ⑧ 全部节点有 entity_source | 来源分类缺失——治理规则无法生效 | `migrateToV4()` |
| ⑨ 全部节点 status 合法 | 状态字段异常——生命周期管理失效 | 批量修正 |
| ⑩ social_group_genes 非空 | 社团基因缺失——自由人至少为"WW" | `_rebuildGroupGenes()` |
...
🔴 分类可变但 TXS-ID 不变：category 列可随人生境遇变化而更新，TXS-ID 终身不变。
```
...
#### Phase 1-5（2026-07-17，✅ 已完成）
- UUID 底座 + category 分类 + migrateToV3-V5
- PAE 引擎 + UUIDGatekeeper + RelationHeatTracker
- entity_source / family_gene / social_group_genes
- fgIntegrityGuard 6→10 项扩展

#### Phase V6（2026-07-18，✅ 已完成）
- PersonDossier 七子卷重构（selfProfile / socialIdentity / _deprecated）
- _changeHistory V6 格式（{time, operation, field, before, after, reason, source}）
- 旧 10 模块归档至 _deprecated
- 39/39 全量审计通过，零结构性问题

### 18.8 完整蓝皮书

详见 [household-registration-blueprint.md](../household-registration-blueprint.md)（户籍制蓝皮书 V4.0，含七子卷明细、10 项守护、运维手册、Phase 6-8 路线图）。